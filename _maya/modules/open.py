...
class MBOpen:
    ...
    def auto_open(self):
        ...
        if self.parent._context.get("type") == "asset":
            listing = self.ui.assets
        else:
            listing = self.ui.shots
    ...
    def open_files(self, it):
        ...
        for file in files:
            ...
            thumbnail = file.get("path").replace(".{}".format(self.parent._project.get("engine").get("ext")), ".jpg")
    ...
    def open_scene(self, it):
        try:
            if self.check_save():
                data = dict(it.data(0, QtCore.Qt.UserRole))
                cmds.file(data.get("path"), force=True, open=True, prompt=False, loadAllReferences=True)
                self._context = data
                ...
                tab_index = self.ui.work_tab.currentIndex()
                if tab_index == 0:
                    self.data_asset.check_data()
                else:
                    self.data_shot.check_data()
        except Exception as error:
            exc_type, exc_obj, exc_tb = sys.exc_info()
            fname = os.path.split(exc_tb.tb_frame.f_code.co_filename)[1]
    ...
    def new_file(self):
        if self.check_save():
            ...
            if self.parent._context.get("type") == "asset" and self.parent._context.get("step") == "Model":
            ...
            if self.parent._context.get("type") == "shot" and self.parent._context.get("step") == "Assembly":
            ...
            if self.parent._context.get("type") == "shot" and self.parent._context.get("step") == "Animation":
