...
class MBLoad:
            ...
            if it.parent():
                ...
                if tab_index == 0:
                    ...
                    if proxy == "proxy":
                        ...
                        path = os.path.join(self.parent._project.get("path"), self.parent._templates.get("shot_publish_cache"))
                    else:
    ...
    def get_files(self, path):
        ...
        if proxy == "proxy":
            ...
            file_path = os.path.join(path, "*", "*", "*.ma")
    ...
    def import_file(self):
        if len(self.ui.publish.selectedItems()) > 0:
            for file in self.ui.publish.selectedItems():
                data = dict(file.data(0, QtCore.Qt.UserRole))
                cmds.file(data.get("path").replace("\\", "/"), i=True, renameAll=True, gr=False, namespace=":", preserveReferences=False)
